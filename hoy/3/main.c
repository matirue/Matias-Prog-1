           ...
           for(j=i+1; j<MAX; j++)
              {
                ...
                if(salario[i] == salario[j])  //al tener igualdad de sueldo se ejecuta ejecuta este if
                  {                           //si quiero ordenar por nombre hago un if para nombre...
                     if(strcmp(nombre[i],nombre[j])>0)
                        {
                          ...
                          /*salarioAux = salario[i];
                            salario[i] = salario[j];
                            salario[j] = salarioAux;*/ //por igualdad no es necesaria esta comparacion
                       }
                }
          }
}
